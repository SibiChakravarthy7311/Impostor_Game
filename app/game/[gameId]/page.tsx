...
export default async function GamePage({ params }: PageProps) {
  ...
  const { data: game } = await supabase
    .from("games")
    .select("id, join_code, status, round_number, winner")
    .eq("id", gameId)
    .maybeSingle();
  ...
  const [{ data: players }, { data: latestRoundEvent }, { data: round }] = await Promise.all([
  ...
  return (
    <section className="space-y-5">
      ...
      {session?.is_host && game.status === "in_progress" ? (
        <section className="card space-y-2">
          <h2 className="text-lg font-semibold">Voting Progress</h2>
          <p className="text-sm text-slate-700">
            {Array.from(votedIds).filter((id) => aliveVoterIds.has(id)).length} / {alivePlayers.length} alive players voted
          </p>
          <ul className="grid gap-1 text-sm sm:grid-cols-2">
            {alivePlayers.map((player) => (
              <li key={player.id} className="rounded border border-slate-200 px-2 py-1">
                {votedIds.has(player.id) ? "[x]" : "[ ]"} {player.name}
              </li>
            ))}
          </ul>
        </section>
      ...
      {game.status === "in_progress" ? (
        <section className="card space-y-4">
          <h2 className="text-xl font-semibold">Round Actions</h2>
          ...
          {canVote ? (
            ...
            <p className="text-sm text-slate-600">Voting unavailable for your current session.</p>
          )}
          ...
          {canKill ? (
            ...
            <p className="text-sm text-slate-600">Lead kill action is available only to the active lead impostor.</p>
          )}
